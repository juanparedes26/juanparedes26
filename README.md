<!-- ✨ Encabezado animado con typing effect -->
<h1 align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=28&duration=3500&pause=1000&color=00BCD4&center=true&vCenter=true&width=700&lines=👋+Hola!+Soy+Juan+Manuel+Paredes;Full+Stack+Developer;Ingeniero+Mecatrónico;Apasionado+por+aprender+y+crear" alt="Typing SVG" />
</h1>

<p align="center">
  <b>Full Stack Developer | Ingeniero Mecatrónico</b><br>
  Apasionado por construir soluciones web funcionales, limpias y con buen diseño.<br>
  Passionate about building functional, clean, and well-designed web solutions.
</p>

---

## 💡 Sobre mí / About me

- 🚀 Desarrollador **Full Stack** enfocado en crear experiencias fluidas de principio a fin.  
  **Full Stack Developer** focused on building smooth end-to-end experiences.  
- 🎓 Ingeniero Mecatrónico en formación constante dentro del mundo del desarrollo.  
  **Mechatronic Engineer**, always learning and evolving in software development.  
- 📚 Actualmente sigo aprendiendo y fortaleciendo mis habilidades en **desarrollo web**.  
  Currently learning and strengthening my **web development** skills.  
- 🧩 Disfruto entender cada parte del código y aprender con propósito.  
  I enjoy understanding every part of the code and learning with purpose.  

---

## 🧰 Stack Tecnológico / Tech Stack

#### ⚛️ Frontend
<p align="left">
  <img src="https://skillicons.dev/icons?i=react,js,html,css,tailwind,bootstrap" />
</p>

#### ⚙️ Backend
<p align="left">
  <img src="https://skillicons.dev/icons?i=nodejs,python,flask,mysql,postgres" />
</p>

#### 🧠 Herramientas / Tools
<p align="left">
  <img src="https://skillicons.dev/icons?i=git,github,vscode" />
</p>

---

## 🧩 Algunos proyectos / Some Projects

| Proyecto / Project | Descripción / Description | Tecnologías / Tech |
|--------------------|---------------------------|--------------------|
| [**SkillMatch**](https://github.com/juanparedes26/Skill_Match2025) | Plataforma de servicios freelance local inspirada en Upwork.<br>Local freelance platform inspired by Upwork. | FastAPI / SQLAlchemy / React |
| [**Proyecto Maquila de Café**](https://github.com/juanparedes26/Proyecto_Maquila_Cafe) | Sistema para la gestión y trazabilidad en el proceso de maquila de café.<br>System for managing and tracking coffee maquila processes. | Python / Flask / MySQL |
| [**Mi Portafolio**](https://github.com/juanparedes26/Mi-portafolio) | Muestra de proyectos, habilidades y CV descargable.<br>Portfolio showcasing projects, skills, and a downloadable CV. | React / TailwindCSS |
| [**Contact List App**](https://github.com/juanparedes26/Juan_Paredes_Contact_List) | Aplicación CRUD para la gestión de contactos.<br>CRUD app for contact management. | Flask / Bootstrap / PostgreSQL |

---

## 📊 Mis estadísticas / My GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=juanparedes26&show_icons=true&theme=tokyonight&hide_border=true" height="160px"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=juanparedes26&layout=compact&theme=tokyonight&hide_border=true" height="160px"/>
</p>

---

## 📫 Conecta conmigo / Get in touch

<p align="center">
  <a href="mailto:juan.paredes260320@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://www.linkedin.com/in/juan-manuel-paredes-lopez-b7621224b">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://github.com/juanparedes26">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>

---

<p align="center">
  <i>"Aprender a construir, construir para aprender."<br>
  "Learn to build, build to learn."</i>
</p>

---

<!-- ✨ Animación de ondas decorativas (opcional) -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=00bcd4&height=100&section=footer"/>
</p>
